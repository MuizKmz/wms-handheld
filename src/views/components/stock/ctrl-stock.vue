...
export default {
  ...
  methods: {
    ...
    async queryInventory1() {
      let inquiredTagSet = new Set([...this.inquiredTags])
      let toInquireTagCodes = Array.from(this.scannedMap.keys()).filter(tagCode => {
        return !inquiredTagSet.has(tagCode)
      })
      if (toInquireTagCodes.length <= 0) {
        return
      }
      let res = await this.$api.queryInventoryList({
        tagCodes: toInquireTagCodes
      })
      // console.log(toInquireTagCodes)
      toInquireTagCodes.forEach(tagCode => {
        let tag = {tagCode: tagCode}
        ...
        tag.tagType = this.scannedMap.get(tagCode)
        tags.splice(0, 0, tag)

        inquiredTagSet.add(tagCode)
      })

      this.inquiredTags = [...inquiredTagSet]
