<template>
  <view v-if="orderProducts.length > 0" class="products-section">
    <view class="product-header">
      <view class="product-title-wrapper">
        <up-icon name="list" size="16" color="#667eea"></up-icon>
        <text class="product-title-text">Products To Stock Out</text>
      </view>
      <view class="product-actions">
        <view class="product-count">
          <text class="count-badge">{{ orderProducts.length }}</text>
        </view>
      ...
      <view class="table-wrapper">
        ...
        >
          ...
          <view class="table-header">
            ...
            <view class="table-cell header-cell qty-col">Stocked-out</view>
          ...
          <view v-for="(item, index) in productList" :key="index" class="table-row">
            ...
            <view class="table-cell qty-col">
              <text class="qty-text" :class="getReceivedClass(item)">{{ getTableText(item.stockOutQuantity) }}</text>
            </view>
            <view class="table-cell qty-col">
...
export default {
  ...
  methods: {
    ...mapActions(useStockStore, {getOrderInfo: 'getOrderAction'}),
    async onReload() {
      this.getOrderInfo()
      this.$msg('Reloaded')
    },
    getReceivedClass(item) {
      if (item.stockOutQuantity < item.expectedQuantity) {
        return 'text-danger'
      } else if (item.stockOutQuantity > item.expectedQuantity) {
        return 'text-warning'
      } else {
        return 'text-success'
      }
