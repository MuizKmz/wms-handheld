 ...
 */
const msg = (title, position = 'center', duration = null, mask = false, icon = 'none') => {
    // 统一提示方便全局修改
    ...
    // Minimum 3000ms, max 6000ms, 100ms per character
    const calculatedDuration = duration || Math.max(3000, Math.min(title.length * 100, 6000))
